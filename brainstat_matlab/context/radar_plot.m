function class_mean = radar_plot(data, varargin)
% RADAR_PLOT    Visualizes data in radar plot.
% 
%   class_mean = radar_plot(data, varargin)
%   visualizes data (data) in a radar plot (author:@saratheriver)
%
%   Inputs:
%       data (double array) - vector of data size [n, 1]
%
%   Valid name-value pairs are:
%       title 
%           (string, optional) - Title of spider plot. 
%           Default is empty.
%       axis_range 
%           (double array, optional) - Range of spider plot axes. 
%           Default is (min, max).
%       label 
%           (cell array, optional) - List of axis labels. Length = 5.
%           Default is names of von Economo-Koskinas cytoarchitectonic 
%           classes.
%       color 
%           (double array, optional) - Color of line. Default is [0 0 0].
%
% Outputs:
...
if isempty(in.label)
    ...
    for ii = 1:length(in.label)
        in.label{ii} = num2str(ii);
    end
end
% Create table
class_mean = array2table(data.', 'VariableNames', in.label);
...
set(gcf, 'units', 'normalized', 'position', [0 0 .33 .33]);
