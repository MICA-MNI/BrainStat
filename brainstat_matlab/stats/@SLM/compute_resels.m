function [resels, reselspvert, edg] = compute_resels(obj)

%Resels of surface or volume data inside a mask.
%
% Usage: [resels, reselspvert, edg] = SurfStatResels( obj [, mask] )
%
...
%             = EC of the mask if obj.resl is not given.
...
% formed only for two adjacent slices in the volume. The obj.lat data is
...
% lkc(1,2)=sum(maskedg);
% if ~isempty(obj.resl)
%     r1=mean(sqrt(obj.resl(maskedg,:)),2);
%     lkc(2,2)=sum(r1);
% end
% 
% %% LKC of triangles
...
% lkc(1,3)=sum(masktri);
% if ~isempty(obj.resl)
%     [tf,loc]=ismember(tri(masktri,[1 2]),edg,'rows');  l12=obj.resl(loc,:);
%     [tf,loc]=ismember(tri(masktri,[1 3]),edg,'rows');  l13=obj.resl(loc,:);
%     [tf,loc]=ismember(tri(masktri,[2 3]),edg,'rows');  l23=obj.resl(loc,:);
...
% 
%         [tf,loc]=ismember(tet(masktet,[1 2]),edg,'rows');  l12=obj.resl(loc,:);
%         [tf,loc]=ismember(tet(masktet,[1 3]),edg,'rows');  l13=obj.resl(loc,:);
%         [tf,loc]=ismember(tet(masktet,[2 3]),edg,'rows');  l23=obj.resl(loc,:);
%         [tf,loc]=ismember(tet(masktet,[1 4]),edg,'rows');  l14=obj.resl(loc,:);
%         [tf,loc]=ismember(tet(masktet,[2 4]),edg,'rows');  l24=obj.resl(loc,:);
%         [tf,loc]=ismember(tet(masktet,[3 4]),edg,'rows');  l34=obj.resl(loc,:);
% 
...
if ~isempty(obj.tri)
    ...
    if isempty(obj.mask)
        v=max(edg(:));
        obj.mask=logical(zeros(1,v));
        obj.mask(edg)=1;
    else
        if size(obj.mask,1)>1
            obj.mask=obj.mask';
        end
        v=size(obj.mask,2);
    end
    ...
    if ~isempty(obj.resl)
        ...
        % obj.resl doesn't exist, hence an error would occur when nargout>1
        % and obj.resl did not exist.
        if nargout>=2
...
if ~isempty(obj.lat)
    ...
    vs=cumsum(squeeze(sum(sum(obj.lat))));
    vs=int32([0; vs; vs(K)]);
    es=0;
    ...
    for k=1:K
        ...
        if f
            ...
            mask1=[obj.mask((vs(k+1)+1):vs(k+2)) obj.mask((vs(k)+1):vs(k+1))];
        end
        ...
        %%
        lkc1=zeros(4);
        lkc1(1,1)=sum(obj.mask((vs(k)+1):vs(k+1)));
        
        %% LKC of edges
        maskedg=all(mask1(edg1),2);
        lkc1(1,2)=sum(maskedg);
        if ~isempty(obj.resl)
            r1=mean(sqrt(obj.resl(find(maskedg)+es,:)),2);
            lkc1(2,2)=sum(r1);
        end
        ...
        if ~isempty(obj.resl)
            if e<2^31
                l12=obj.resl(sparsedg(tri(masktri,1)+m1*(tri(masktri,2)-1),1)+es,:);
                l13=obj.resl(sparsedg(tri(masktri,1)+m1*(tri(masktri,3)-1),1)+es,:);
                l23=obj.resl(sparsedg(tri(masktri,2)+m1*(tri(masktri,3)-1),1)+es,:);
            else
                l12=obj.resl(interp1(ue,ae,tri(masktri,1)+m1*(tri(masktri,2)-1),'nearest')+es,:);
                l13=obj.resl(interp1(ue,ae,tri(masktri,1)+m1*(tri(masktri,3)-1),'nearest')+es,:);
                l23=obj.resl(interp1(ue,ae,tri(masktri,2)+m1*(tri(masktri,3)-1),'nearest')+es,:);
            end
            a=max(4*l12.*l13-(l12+l13-l23).^2,0);
            r2=mean(sqrt(a),2)/4;
        ...
        if ~isempty(obj.resl) & k<K
            if e<2^31
                l12=obj.resl(sparsedg(tet(masktet,1)+m1*(tet(masktet,2)-1),1)+es,:);
                l13=obj.resl(sparsedg(tet(masktet,1)+m1*(tet(masktet,3)-1),1)+es,:);
                l23=obj.resl(sparsedg(tet(masktet,2)+m1*(tet(masktet,3)-1),1)+es,:);
                l14=obj.resl(sparsedg(tet(masktet,1)+m1*(tet(masktet,4)-1),1)+es,:);
                l24=obj.resl(sparsedg(tet(masktet,2)+m1*(tet(masktet,4)-1),1)+es,:);
                l34=obj.resl(sparsedg(tet(masktet,3)+m1*(tet(masktet,4)-1),1)+es,:);
            else
                l12=obj.resl(interp1(ue,ae,tet(masktet,1)+m1*(tet(masktet,2)-1),'nearest')+es,:);
                l13=obj.resl(interp1(ue,ae,tet(masktet,1)+m1*(tet(masktet,3)-1),'nearest')+es,:);
                l23=obj.resl(interp1(ue,ae,tet(masktet,2)+m1*(tet(masktet,3)-1),'nearest')+es,:);
                l14=obj.resl(interp1(ue,ae,tet(masktet,1)+m1*(tet(masktet,4)-1),'nearest')+es,:);
                l24=obj.resl(interp1(ue,ae,tet(masktet,2)+m1*(tet(masktet,4)-1),'nearest')+es,:);
                l34=obj.resl(interp1(ue,ae,tet(masktet,3)+m1*(tet(masktet,4)-1),'nearest')+es,:);
            end
