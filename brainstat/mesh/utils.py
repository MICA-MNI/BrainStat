...
def mesh_edges(surf):
    ...
    # Convert triangles to edges by grabbing all unique edges within triangles.
        ...
        for f in range(0, 2):
            ...
            c11 = np.where((np.remainder((i+j), 2) == f)
                           & (i == I) & (j < J))[0]
            c21 = np.where((np.remainder((i+j), 2) == f)
                           & (i == I) & (j > 1))[0]
            c12 = np.where((np.remainder((i+j), 2) == f)
                           & (i < I) & (j == J))[0]
            c22 = np.where((np.remainder((i+j), 2) == f)
                           & (i > 1) & (j == J))[0]
        ...
        for j in range(0, filenames.shape[1]):
            ...
            if j is 0:
                ...
                coord = np.concatenate(
                    (coord, get_points(surfaces[j])), axis=0)
